 ...
 */
public class StudentForm implements MonkeyScreen
{
  ...
  public StudentForm(Main _reference)
  {
  ...
  void myorar(String zi)
  {
    ...
    try {
      ...
      while(!result[contor].isEmpty())
      {
        ...
        String grupid = dataManager.ReadCol("dateuser","id_grupa","WHERE `id` ='"
            + reference.CONNECTED_ID + "';");
        String curs = dataManager.ReadCol("cursuri","nume","WHERE `id` = '"
                + name[0] + "' AND `id_grupa` = '"+ grupid + "';");
        if(!curs.isEmpty()) {
          String AUX = "Nume:  " + curs + "           Ora_intrare:  " + name[1];
